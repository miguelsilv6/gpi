...
export function LocaisTratamentoTab() {
  const [locais, setLocais] = useState<LocalTratamento[]>([])
  ...
  const [deleteCandidate, setDeleteCandidate] = useState<LocalTratamento | null>(null)
  const [deleting, setDeleting] = useState(false)

  async function load() {
    setLoading(true)
    const res = await fetch('/api/locais-tratamento')
  ...
  async function handleAdd() {
    ...
    if (!res.ok) {
      ...
      toast.error(err.error ?? 'Erro ao criar local de tratamento')
      return
    }
    toast.success('Local de tratamento criado')
    setNeu(EMPTY_NEW)
    setAdding(false)
    load()
  }

  async function handleToggleAtivo(c: LocalTratamento) {
    const res = await fetch(`/api/locais-tratamento/${c.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ativo: !c.ativo }),
    })
    if (res.ok) {
      setLocais((prev) => prev.map((x) => (x.id === c.id ? { ...x, ativo: !x.ativo } : x)))
    } else {
      const err = await res.json().catch(() => ({}))
  ...
  function startEdit(c: LocalTratamento) {
    setEditId(c.id)
  ...
  async function handleEditSave(id: string) {
    ...
    const res = await fetch(`/api/locais-tratamento/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        nome: edit.nome.trim(),
    ...
    setLocais((prev) => prev.map((x) => (x.id === id ? updated : x)))
    setEditId(null)
  ...
  async function handleDeactivate() {
    ...
    setDeleting(true)
    const res = await fetch(`/api/locais-tratamento/${deleteCandidate.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    toast.success('Local de tratamento desativado')
  ...
  async function handleHardDelete() {
    if (!deleteCandidate) return
    setDeleting(true)
    const res = await fetch(`/api/locais-tratamento/${deleteCandidate.id}`, { method: 'DELETE' })
    setDeleting(false)
    if (!res.ok) {
      const err = await res.json().catch(() => ({}))
    ...
    setLocais((prev) => prev.filter((x) => x.id !== deleteCandidate.id))
    toast.success('Local de tratamento eliminado')
    setDeleteCandidate(null)
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Catálogo de locais de tratamento dos inquéritos. Apenas locais ativos aparecem na seleção.
        </p>
        {!adding && (
      ...
      {locais.length === 0 && !adding ? (
        <p className="text-sm text-muted-foreground text-center py-8">
          Nenhum local de tratamento configurado. Adicione um para começar.
        </p>
      ) : (
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Apagar local de tratamento</DialogTitle>
          </DialogHeader>
          {deleteCandidate && (
            <div className="space-y-3 text-sm">
              ...
              <div className="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-900 p-3 space-y-2">
                ...
                <p className="text-xs text-amber-900/80 dark:text-amber-200/80">
                  O local de tratamento deixa de aparecer na criação de novos inquéritos, mas os
                  inquéritos existentes mantêm a referência intacta.
                </p>
                <Button
                  size="sm"
              ...
              <div className="rounded-lg border border-red-200 bg-red-50 dark:bg-red-950/20 dark:border-red-900 p-3 space-y-2">
                ...
                <p className="text-xs text-red-900/80 dark:text-red-200/80">
                  ...
                  associados a este local — nesse caso desative em vez de eliminar.
                </p>
                <Button
                  size="sm"
                  variant="destructive"
