...
export const queryBrigadas: RelatorioHandler = async (filters, session) => {
  ...
  return {
    ...
    columns: [
      { key: 'brigada', label: 'Brigada', flex: 2.0 },
      { key: 'abertosPeriodo', label: 'Abertos no período', align: 'right', flex: 0.7 },
      ...
      { key: 'aguarda', label: 'Aguarda exames', align: 'right', flex: 0.65 },
      { key: 'enviados', label: 'Enviados', align: 'right', flex: 0.6 },
      { key: 'vencidos', label: 'Prazos vencidos', align: 'right', flex: 0.65 },
    ],
    rows,
