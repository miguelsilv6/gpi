 ...
 *   - inspetorId
 *
 * Scope: respeita `buildInqueritoWhere`.
 */
export const queryCartasPrecatorias: RelatorioHandler = async (filters, session) => {
  ...
  const rows: RelatorioRow[] = inqueritos.map((i) => ({
    ...
    tribunal: i.tribunal?.nome ?? '',
    seccao: i.seccao?.nome ?? '',
  ...
  return {
    title: 'Relatório de Cartas Precatórias',
    geradoEm: new Date(),
    geradoPor: session.nome,
    filtros: {
      'Tipo': 'Cartas Precatórias',
    ...
    columns: [
      ...
      { key: 'crime', label: 'Crime / Natureza', flex: 1.4 },
      { key: 'estado', label: 'Estado', flex: 0.9 },
      { key: 'brigada', label: 'Brigada', flex: 1.0 },
      { key: 'inspetor', label: 'Inspetor', flex: 1.0 },
      { key: 'tribunal', label: 'Tribunal', flex: 1.1 },
      { key: 'seccao', label: 'Secção', flex: 1.0 },
    ...
    rows,
    summary,
    emptyMessage: 'Nenhuma carta precatória corresponde aos filtros aplicados.',
  }
}
