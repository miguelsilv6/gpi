...
export const NOTIFICATION_TIPO_LABELS: Record<TipoNotificacao, string> = {
  ...
  INTERCECAO_VALIDACAO_APROXIMA: 'Validação de interceções a aproximar-se',
  INTERCECAO_RENOVACAO_PREPARAR: 'Renovação de interceção a preparar',
...
export const NOTIFICATION_TIPO_DESCRIPTIONS: Record<TipoNotificacao, string> = {
  ...
  INTERCECAO_VALIDACAO_APROXIMA: 'Disparada antes de cada validação quinzenal das interceções (art. 188.º CPP) que ainda não foi marcada como feita, com os dias de antecedência definidos no plano do inquérito (por defeito 3). Notifica o inspetor do inquérito.',
  INTERCECAO_RENOVACAO_PREPARAR: 'Disparada na validação que antecede o fim de uma interceção — é nessa apresentação que a renovação tem de ser pedida. Indica as linhas em causa e as respetivas datas de fim. Notifica o inspetor do inquérito.',
...
export const NOTIFICATION_TIPO_HAS_NATURAL: Record<TipoNotificacao, boolean> = {
  ...
  INTERCECAO_VALIDACAO_APROXIMA: true,
  INTERCECAO_RENOVACAO_PREPARAR: true,
